...
GPU_ID="${GPU_ID:-1}"
LOG_DIR="${LOG_DIR:-$ROOT_DIR/logs/moon_cifar10_task5}"
CIFAR100_LOG_DIR="${ROOT_DIR}/logs/cifar100_download"
mkdir -p "$LOG_DIR" "$CIFAR100_LOG_DIR"
...
echo "[$(date '+%F %T')] Step 2/3: Run MOON task-5 (5 runs) on GPU ${GPU_ID}"
GPU_ID="${GPU_ID}" bash "${ROOT_DIR}/scripts/run_moon_cifar10_task5_5runs.sh"

echo "[$(date '+%F %T')] MOON task-5 runs finished. Waiting for CIFAR-100 job (PID=${CIFAR100_PID})..."
