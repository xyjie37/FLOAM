...
# 1) Split CIFAR-10 into task-10
# 2) Download/split CIFAR-100 in background (CPU/network)
# 3) Run FedAvg task-10 x5 on GPU 1
...
GPU_ID="${GPU_ID:-1}"
LOG_DIR="${LOG_DIR:-$ROOT_DIR/logs/fedavg_cifar10_task10}"
CIFAR100_LOG_DIR="${ROOT_DIR}/logs/cifar100_download"
mkdir -p "$LOG_DIR" "$CIFAR100_LOG_DIR" /home/jxy/data/cifar100

SPLIT_OUT="${ROOT_DIR}/dataset/cifar10-dir-0.1-task-10"

echo "[$(date '+%F %T')] Step 1/3: Split CIFAR-10 into task-10 + start CIFAR-100 download in parallel"
...
if [[ -d "${SPLIT_OUT}/train" && -d "${SPLIT_OUT}/test" ]]; then
  ...
  cd "${ROOT_DIR}/dataset"
  NUM_TASK=10 DATASET_ROOT=/home/jxy/data/cifar10 \
    "$PYTHON" split_data_cifar10.py > "${LOG_DIR}/split_task10.log" 2>&1
  cd "$ROOT_DIR"
  echo "  Split done -> ${SPLIT_OUT}"
fi

echo "[$(date '+%F %T')] Step 2/3: Run FedAvg task-10 (5 runs) on GPU ${GPU_ID}"
GPU_ID="${GPU_ID}" bash "${ROOT_DIR}/scripts/run_fedavg_cifar10_task10_5runs.sh"

echo "[$(date '+%F %T')] FedAvg task-10 runs finished. Waiting for CIFAR-100 job (PID=${CIFAR100_PID})..."
