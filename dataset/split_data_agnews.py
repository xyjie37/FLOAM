...
print(f"AG News dataset with {num_classes} classes")
print(f"Classes: {class_names}")

datasetroot_dir = "./agnews"
...
def load_ag_news_data(train_file, test_file, max_features=20000, max_len=200):
    """
    Load and preprocess AG News dataset
    Expected format: CSV with columns [class, title, description]
    """
    ...
    # Convert class labels to integers and then to 0-based indexing
    train_df['class'] = pd.to_numeric(train_df['class'], errors='coerce').astype(int) - 1
    test_df['class'] = pd.to_numeric(test_df['class'], errors='coerce').astype(int) - 1
...
X_train, y_train, X_test, y_test, vocab_size = load_ag_news_data(train_file, test_file)
