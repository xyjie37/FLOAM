...
Client-side: supervised CE + model-level contrastive loss that pulls the
current local representation toward the global model and pushes it away
from the previous local model.
"""
...
from models.Update import LocalUpdateMOON
...
if __name__ == '__main__':
    ...
    # Previous local models used as contrastive negatives (first round: None)
    prev_local_list = [None for _ in range(args.num_users)]
    ...
    for iter in range(args.epochs):
        ...
        # Frozen global model as contrastive positive
        global_net = copy.deepcopy(net_glob)
        global_net.eval()
        ...
        for idx in idxs_users:
            local = LocalUpdateMOON(args=args, dataset=dataset_path, idxs=idx, task=task)
            net_local = copy.deepcopy(net_local_list[idx])

            prev_net = None
            if prev_local_list[idx] is not None:
                prev_net = copy.deepcopy(prev_local_list[idx]).to(args.device)
                prev_net.eval()
            ...
            # Save trained local model as the negative for the next participation
            prev_local_list[idx] = copy.deepcopy(net_local).cpu()
